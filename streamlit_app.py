...
with tab1:
    ...
    vacancy_rate = (round((vacant_units / total_units) * 100),2).astype(str) + '%'  # 空租率
...
# 将按类型分组的空房间数量与总房间数量合并
    ...
    vacant_by_type['Vacancy Rate'] = (round((vacant_by_type['Vacant Units'] / vacant_by_type['Total Units']) * 100),2).astype(str) + '%'
